...
loginButton.addEventListener('click', function(e) {
    ...
    if (username === 'User77' && password === '123456789') {
        ...
        window.location.href = `Home.html?username=${username}&password=${password}&ip_address=${ipAddress}`;
    } else if (username === 'Admin77' && password === '1985127') {
        // Redirect the admin to the appropriate page
        window.location.href = `Home.html?username=${username}&password=${password}&ip_address=${ipAddress}`;
        window.location.href = `Control-Panel.html?username=${username}&password=${password}&ip_address=${ipAddress}`;
    } else {
